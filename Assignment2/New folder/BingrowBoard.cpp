...
ostream& operator<<(ostream& os, BingrowSquare &sq){
   ...
   else if( sq.getSymbol().getSymbolType() == Number){
      if(sq.getCovered() == true){
         os<<"[X]";
      }
      else{
...
bool BingrowBoard::WinningBoard(){
   ...
   for(int rO = 0; rO<currentSize; rO++){
      ...
      if(theboard[rO][rO].getCovered() == true){
         ...
         //cout<<"rO,rO"<<rO<<','<<rO<<endl;
      }
      //cout<<"rT,rT"<<rT<<','<<rT<<endl;
      if(theboard[rO][rT].getCovered() == true){
         cout<<"rO,rT "<<rO<<','<<rT<<endl;
...
void BingrowBoard::GrowBoard(ArrowType direction){
   cout<<"GrowBoard Function has been called: ";
   vector<BingrowSquare> squareRow;
   ...
   if(direction == UpperLeft && maxULArrow<2){
      ...
      --leftValues;
      cout<<"Board has grown to UpperLeft \n"<<endl;
   }
   if(direction == UpperRight && maxURArrow<2){
      ...
      theboard.insert(theboard.begin(), squareRow);//adding the row of coloumn elements first row
      cout<<"Board has grown to UpperRight \n"<<endl;
   }
   if(direction == LowerLeft && maxLLArrow<2){
      ...
      --leftValues;
      cout<<"Board has grown to LowerLeft \n"<<endl;   
   }
   if(direction == LowerRight && maxLRArrow<2){
      ...
      cout<<"Board has grown to LowerRight \n"<<endl;
   }
...
void BingrowBoard::printBoard(){
   cout << "-------"<< endl;
   for( int r = 0; r < currentSize; r++ ){//looping through rows
                //column restarts counting
      for( int c = 0; c < currentSize; c++ ){ //looping through columns theboard[r][c];
         cout << theboard[r][c]<<" \t ";
      }
      cout << endl;
   }
   cout << "-------\ncurrentSize: "<< currentSize <<", originalSize: "<<originalSize<< endl;
   cout << "Bingrow Size: "<<currentSize<<"x"<<currentSize<<"\n-------" << endl;
}
...
int main(){
   ...
   // Symbol symb;
   // symb.setNumber(5);
   // symb.setSymbolType(Number);

   // BingrowSquare sq;
   // sq.setSymbol(symb);
   // sq.setCovered(false);

   // Bingo.SetSquare(0,0, sq);

   // Bingo.printBoard();
   
   string run = "";
   while( !(run=="N" || run=="n") && !(Bingo.WinningBoard()) ){
      cout<<"Do you want to cover another square? Write (Y/N): ";
      cin>>run; 
      cout<<endl;
      cout<<"Choose a number of the following: \n 1) A number\n 2) An Arrow"<<endl;
      int input;
      cin>> input;
      if ( input == 1){
         Symbol symbol;
         cout<<"Write a number on the board: "; 
         cin>> input; cout<<endl;
         symbol.setSymbolType(Number);
         symbol.setNumber(input);
         Bingo.UpdateBoard(symbol);
      }
      else if( input == 2){
         Symbol symbol;
         cout<<"Write an Arrow Type number from\n 0) UpperLeft\n 1) UpperRight\n 2) LowerLeft\n 3) LowerRight"<<endl;
         int input;
         cin>> input; cout<<endl;
         ArrowType arrowType = static_cast<ArrowType>( input );
         symbol.setSymbolType(Arrow);
         symbol.setArrowType(arrowType);
         Bingo.UpdateBoard(symbol);
      }
   }
   return 0;
